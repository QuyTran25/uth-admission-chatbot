...
#### Mã nguồn sơ đồ Mermaid (Dùng để hiển thị và lưu vết thay đổi trên Git)

```mermaid
graph TD
    %% Định nghĩa các Style
    classDef user fill:#e1f5fe,stroke:#03a9f4,stroke-width:2px;
    classDef fe fill:#e8f5e9,stroke:#4caf50,stroke-width:2px;
    classDef be fill:#fff3e0,stroke:#ff9800,stroke-width:2px;
    classDef db fill:#ede7f6,stroke:#673ab7,stroke-width:2px;
    classDef external fill:#fce4ec,stroke:#e91e63,stroke-width:2px;
    classDef error fill:#ffebee,stroke:#ef5350,stroke-width:2px;

    %% Định nghĩa các node
    UserInput["Người dùng - Thí sinh và Phụ huynh"]:::user
    WebUI["Giao diện Chat UI - React"]:::fe
    GenAPI["Generation API Backend - FastAPI"]:::be
    RetrievalAPI["Retrieval API Backend - FastAPI"]:::be
    
    subgraph OfflinePrep ["Offline - Chuẩn bị dữ liệu"]
        RawData["Tài liệu PDF/Web"]:::db
        DoclingProc["Docling - Layout & Table"]:::be
        TablePipeline["Pipeline 6 bước xử lý bảng"]:::be
        KVChunking["KV-Chunking"]:::be
        SpotCheck["Spot-check thủ công - verified=true"]:::be
        FAISS_Idx["FAISS Index"]:::db
        BM25_Idx["BM25 Index"]:::db
    end

    subgraph YearFilterStage ["Xử lý năm tuyển sinh & Phân loại"]
        ExtractParams["Trích xuất Năm & Loại tài liệu"]:::be
        CheckYearBound{"Năm tuyển sinh?"}:::be
        CheckDocType{"Phân loại tài liệu?"}:::be
        CheckYearForCutoff{"Năm được nêu?"}:::be
        CheckYearForOthers{"Năm được nêu?"}:::be
        ClarifyYear["clarification_needed - Yêu cầu chọn năm"]:::be
        Default2026["Mặc định filter_year = 2026"]:::be
        Pre2022Refusal["YEAR_NOT_SUPPORTED (Năm < 2022)"]:::be
        FutureYearRefusal["YEAR_NOT_SUPPORTED (Năm > 2026)"]:::be
        UnsupportedYearRefusal["YEAR_NOT_SUPPORTED (Tài liệu khác chỉ hỗ trợ năm 2026)"]:::be
    end

    subgraph RetrievalGateStage ["Retrieval Gate"]
        HybridRetriever["Hybrid Retrieval - BM25 + FAISS"]:::be
        RetrievalGate{"Đạt ngưỡng 4 tín hiệu?"}:::be
        RefusedScope["refused - OUT_OF_SCOPE"]:::be
        SystemError1["error - HTTP 500 (chỉ mục/db lỗi)"]:::error
    end

    subgraph GenAttributionStage ["Generation & Attribution Gate"]
        PromptBuilder["Prompt Builder + history"]:::be
        GeminiAPI["Gemini API"]:::external
        AttributionGate{"Số liệu khớp header-value?"}:::be
        SuccessResponse["Trả câu trả lời xác thực"]:::be
        RefusedGen["refused - sai lệch/bịa số"]:::be
        SystemError2["error - HTTP 500 (Gemini timeout)"]:::error
    end

    %% Kết nối Offline
    RawData --> DoclingProc
    DoclingProc --> TablePipeline
    TablePipeline --> KVChunking
    KVChunking --> SpotCheck
    SpotCheck -->|Đã kiểm định| FAISS_Idx
    SpotCheck -->|Đã kiểm định| BM25_Idx

    %% Kết nối Online
    UserInput -->|1. Gửi câu hỏi| WebUI
    WebUI -->|2. HTTP Request /api/v1/chat| GenAPI
    GenAPI -->|3. Gọi /api/v1/retrieve| RetrievalAPI

    %% Nhánh xử lý năm
    RetrievalAPI --> ExtractParams
    ExtractParams --> CheckYearBound
    
    CheckYearBound -->|Năm < 2022| Pre2022Refusal
    CheckYearBound -->|Năm > 2026| FutureYearRefusal
    CheckYearBound -->|Không nêu năm hoặc năm trong [2022, 2026]| CheckDocType
    
    CheckDocType -->|Điểm chuẩn (cutoff_score)| CheckYearForCutoff
    CheckDocType -->|Tài liệu khác (học phí, chỉ tiêu...)| CheckYearForOthers
    
    CheckYearForCutoff -->|Có năm| HybridRetriever
    CheckYearForCutoff -->|Không năm| ClarifyYear
    
    CheckYearForOthers -->|Có năm = 2026| HybridRetriever
    CheckYearForOthers -->|Không năm (mặc định)| Default2026
    CheckYearForOthers -->|Có năm != 2026| UnsupportedYearRefusal

    Default2026 --> HybridRetriever

    %% Kết nối từ RetrievalAPI về GenAPI (Không đi tắt về WebUI)
    ClarifyYear -->|Trả status & options| GenAPI
    Pre2022Refusal -->|Trả YEAR_NOT_SUPPORTED| GenAPI
    FutureYearRefusal -->|Trả YEAR_NOT_SUPPORTED| GenAPI
    UnsupportedYearRefusal -->|Trả YEAR_NOT_SUPPORTED| GenAPI
    RefusedScope -->|Trả OUT_OF_SCOPE| GenAPI
    SystemError1 -.->|Trả SYSTEM_ERROR 500| GenAPI

    %% Nhánh retrieval
    HybridRetriever -->|Đọc Index| FAISS_Idx
    HybridRetriever -->|Đọc Index| BM25_Idx
    HybridRetriever --> RetrievalGate
    HybridRetriever -.->|Lỗi kỹ thuật| SystemError1

    RetrievalGate -->|Đạt| GenAPI
    RetrievalGate -->|Không đạt| RefusedScope

    %% Nhánh Generation
    GenAPI --> PromptBuilder
    PromptBuilder --> GeminiAPI
    GeminiAPI --> AttributionGate
    GeminiAPI -.->|Lỗi Gemini| SystemError2

    AttributionGate -->|Khớp| SuccessResponse
    AttributionGate -->|Không khớp| RefusedGen

    %% Trả kết quả từ GenAPI về WebUI
    SuccessResponse -->|Trả kết quả hiển thị| WebUI
    RefusedGen -->|Từ chối hiển thị| WebUI
    SystemError2 -.->|Trả lỗi 500| WebUI
    GenAPI -.->|Chuyển tiếp lỗi/hỏi lại từ Retrieval| WebUI
```
